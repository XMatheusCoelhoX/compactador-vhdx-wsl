...
echo try { if (-not $killerProc.HasExited) { $killerProc.Kill() } } catch {} >>"%DPHELPER%"
echo if (-not $proc.HasExited) { try { $proc.Kill() } catch {}; $hang = $true; Start-Sleep -Milliseconds 500 } >>"%DPHELPER%"
